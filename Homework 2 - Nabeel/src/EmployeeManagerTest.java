...
public class EmployeeManagerTest
{
	...
	{
		assertEquals("Nabeel", empMgr.employees().peekElementAt(1).first);
		assertEquals("John", empMgr.employees().peekElementAt(2).first);
		assertEquals("Brad", empMgr.employees().peekElementAt(3).first);
		assertEquals("Matt", empMgr.employees().peekElementAt(4).first);
		assertEquals("Alice", empMgr.employees().peekElementAt(5).first);	
		
		assertEquals(5, empMgr.companySize(),.001);	
		
		empMgr.addEmployee(new Employee("Kim", "Lee", "Manager",40.00, 3, 35, "Female"));
		assertEquals("Kim", empMgr.employees().peekElementAt(6).first);	
	}
	...
	public void testReadFile() {
		 ...
		 EmployeeManager mgr = new EmployeeManager();
			...
			assertEquals("Marc", mgr.employees().peekElementAt(1).first); // The first employee in the TestInput.txt file should be "Marc"
			assertEquals("Sean", mgr.employees().peekElementAt(2).first); // The second employee in the TestInput.txt file should be "Sean"
	...
	public void testWriteFile() {
		 ...
		 mgr.addEmployee(emp2);
			...
			assertEquals("Bill", mgr.employees().peekElementAt(1).first);   // The first employee written into the Testoutput.txt file should be "Bill"
			assertEquals("Austin", mgr.employees().peekElementAt(2).first); // The first employee written into the Testoutput.txt file should be "Austin"
